...
export const getColumns = (): ColumnDef<ProcessedResult>[] => [
  ...
  },
  {
    accessorKey: 'id',
    header: 'Result ID',
    cell: (info) => <span className="font-semibold text-slate-900">{info.getValue() as number}</span>,
  },
  {
    accessorKey: 'userId',
    header: 'User ID',
      ...
      return (
        <span
          className={`mx-auto inline-flex items-center rounded-full px-2.5 py-1 text-xs font-semibold ${
            status === 'IN RANGE' ? 'bg-emerald-50 text-emerald-700' : 'bg-rose-50 text-rose-700'
          }`}
        >
          {status}
        </span>
